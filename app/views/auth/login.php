...
<body class="bg-[#38bdf8] font-sans min-h-screen flex items-center justify-center p-6">

    <div class="bg-white neo-box p-8 max-w-md w-full rounded-sm">
        <h2 class="text-4xl font-black mb-6 uppercase tracking-tight text-center">Login</h2>
        
        <?php if(!empty($error)): ?>
            <div class="bg-red-400 neo-box p-3 mb-4 text-black font-bold"><?= $error ?></div>
        <?php endif; ?>

        <form action="/taskmaster/auth/login" method="POST" class="space-y-5">
            <div>
            ...
            <div>
                ...
                <input type="password" name="password" required class="w-full p-3 neo-input bg-gray-50 rounded-sm">
            </div>
            <button type="submit" class="w-full bg-[#a3e635] text-black font-black text-xl py-3 neo-button mt-4 rounded-sm">
                MASUK
            </button>
        </form>
        <p class="mt-6 text-center font-bold">
            Belum punya akun? <a href="/taskmaster/auth/register" class="text-blue-600 underline hover:text-blue-800">Daftar sekarang</a>
        </p>
    </div>
